...
import { PKG_COMP_PREFIX } from '../utils/consts'
import {
  epOutput,
  epPackage,
  getPackageManifest,
...
const reComponentName: ReComponentName = (title) => {
  return `${PKG_COMP_PREFIX}-${hyphenate(removeTag(title)).replace(/[ ]+/g, '-')}`
}
...
const reWebTypesSource: ReWebTypesSource = (title) => {
  const symbol = `Sc${removeTag(title)
    .replaceAll(/-/g, ' ')
...
const reAttribute: ReAttribute = (value, key) => {
  ...
  if (key === 'Name' && /^(-|—)$/.test(str)) {
    ...
    return str
      ? `${PKG_COMP_PREFIX}-${str
          .replaceAll(/\s*\/\s*/g, `/${PKG_COMP_PREFIX}-`)
          .replaceAll(/\B([A-Z])/g, '-$1')
          .replaceAll(/\s+/g, '-')
          .toLowerCase()}`
      : undefined
...
export const buildHelper: TaskFunction = (done) => {
  ...
  const _version = tagVer
    ? tagVer.startsWith('v')
      ...
      : tagVer
    : version!
  let entry = `${path.resolve(projRoot, 'docs/zh-CN/component')}/**.md`
